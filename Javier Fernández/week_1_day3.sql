...
/* Corrección Javi Fernández */

SELECT CUSTOMER_ID, ARRAY_AGG(IFNULL(PRODUCT_NAME,'NO FIRST ORDER')) AS FIRST_ORDER
FROM (
    SELECT M.CUSTOMER_ID, MEN.PRODUCT_NAME
    FROM MEMBERS M
    LEFT JOIN SALES S
        ON M.CUSTOMER_ID = S.CUSTOMER_ID
    INNER JOIN (SELECT CUSTOMER_ID, MIN(ORDER_DATE) AS FIRST_ORDER_DATE FROM SALES GROUP BY CUSTOMER_ID) AS MIN_DATE
        ON S.CUSTOMER_ID = MIN_DATE.CUSTOMER_ID AND S.ORDER_DATE = MIN_DATE.FIRST_ORDER_DATE
    LEFT JOIN MENU MEN
        ON S.PRODUCT_ID = MEN.PRODUCT_ID
    GROUP BY M.CUSTOMER_ID, MEN.PRODUCT_NAME
)
