/*Día 5 Transformar las opciones del día anterior a funciones de usuario.

Al principio me equivoque y cree procedimientos (lo se. Es lo que tiene ir con prisa). 
Lo bueno es que no solo he visto como se hace, si no que después de mucha batalla, han 
las cosas.

Al final tienes todos los ejemplos que se hen ocurrido.
...
CREATE OR REPLACE FUNCTION balance(cliente INTEGER, primer_dia DATE, ultimo_dia DATE) 
RETURNS INTEGER
LANGUAGE SQL
AS
$$
    SELECT 
        COALESCE(SUM(COALESCE(D, 0) - COALESCE(P, 0) - COALESCE(W, 0)), 0) AS total
    FROM (
        SELECT
...
CREATE OR REPLACE FUNCTION deposito(cliente INTEGER, primer_dia DATE, ultimo_dia DATE) 
RETURNS INTEGER
LANGUAGE SQL
AS
$$
    SELECT 
        COALESCE(SUM(C.TXN_AMOUNT),0) AS total
    FROM 
...
CREATE OR REPLACE FUNCTION compra(cliente INTEGER, primer_dia DATE, ultimo_dia DATE) 
RETURNS INTEGER
LANGUAGE SQL
AS
$$
...
CREATE OR REPLACE FUNCTION retiro(cliente INTEGER, primer_dia DATE, ultimo_dia DATE)
RETURNS INTEGER
LANGUAGE SQL
AS
$$
  ...
  WHERE 
    ...
    AND TXN_DATE BETWEEN primer_dia AND ultimo_dia
$$
;
...
BEGIN
    ...
    CASE 
        ...
        ELSE
            ...
            CASE 
                WHEN tipo = 'B' THEN
					SELECT balance(:cliente, :primer_dia, :ultimo_dia) INTO total;
                WHEN tipo = 'D' THEN
                    SELECT deposito(:cliente, :primer_dia, :ultimo_dia) INTO total;
                WHEN tipo = 'C'THEN
                    SELECT compra(:cliente, :primer_dia, :ultimo_dia) INTO total;
                WHEN tipo = 'R' THEN
                    SELECT retiro(:cliente, :primer_dia, :ultimo_dia) INTO total;
            END CASE;
