...
/*QUERY TRAS COMENTARIOS DEL DIA 2 (HE MODIFICADO LAS UNIDADES PARA QUE APAREZCAN EN LA CABECERA DE LA COLUMNA)*/
/*QUERY TRAS COMENTARIOS DEL DIA 2 (HE MODIFICADO LAS UNIDADES PARA QUE APAREZCAN EN LA CABECERA DE LA COLUMNA)*/
WITH CTE_PRICES AS(
    /*COLUMNA DE PRECIOS DE PIZZAS*/
    SELECT pizza_id,
    ...
    /*CALCULAMOS LOS TOTALES*/
    SELECT 'TOTAL' totales,
        SUM(DD.pizza_sales) total_sales,
        SUM(EE.runner_cost) total_cost,
        SUM(DD.pizza_sales) - SUM(EE.runner_cost) total_profit
    FROM CTE_DELIVERED DD
    RIGHT JOIN CTE_RUNNER_STATS EE
        ON DD.runner_id=EE.runner_id
)
...
SELECT TO_VARCHAR(D.runner_id) RUNNER,
    D.pizza_sales "SALES(€)",
    E.runner_cost "RUNNER_COST(€)",
    D.pizza_sales - E.runner_cost "PROFIT(€)"
FROM CTE_DELIVERED D
RIGHT JOIN CTE_RUNNER_STATS E
    ON D.runner_id=E.runner_id
UNION ALL
