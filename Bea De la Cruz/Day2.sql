...
El resultado no es del todo correcto, añadiría un DISTINCT al count para no repetir dias. La utilización de FULL OUTER JOIN no lo es, en este caso en concreto con pocos registros que cruzar no supone un problema de rendimiento pero en casos con millones de registros el
...
select
      a.customer_id
    , count(distinct b.order_date) as num_visitas
from members a
left join sales b
